...
public record SQLWorker(SQLConnector sqlConnector) {
    ...
    public List<String> getEntriesForPerson(String person) {
        ArrayList<String> arrayList = new ArrayList<>();
        try (ResultSet resultSet = querySQL("SELECT URL FROM `URL` WHERE PERSON =?", person)) {
            while (resultSet != null && resultSet.next()) arrayList.add(resultSet.getString("URL"));
        } catch (Exception ignored) {}
        return arrayList;
    }
    ...
    public ResultSet querySQL(String sqlQuery, Object... objcObjects) {
        ...
        try (PreparedStatement preparedStatement = sqlConnector.getConnection().prepareStatement(sqlQuery)) {
            ...
            if (exception instanceof SQLNonTransientConnectionException) {
                Main.getInstance().getLogger().severe("Couldn't send Query to SQL-Server, most likely a connection Issue.\n" + exception.getMessage());
                sqlConnector.connectToSQLServer();
            } else {
                Main.getInstance().getLogger().severe("Couldn't send Query to SQL-Server ( " + sqlQuery + " )\n" + exception.getMessage());
            }
        }
